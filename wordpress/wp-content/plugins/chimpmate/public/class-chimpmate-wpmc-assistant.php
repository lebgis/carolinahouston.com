...
class ChimpMate_WPMC_Assistant {
	...
	private function __construct() {
		...
		$this->orig_set = get_option('wpmchimpa_options');
		add_action( 'init', array( $this, 'load_plugin_textdomain' ) );
		add_action( 'init', array( $this, 'integrateWithVC' ) );
		add_action( 'template_redirect', array( $this, 'loader' ) );
		add_action( 'wpmu_new_blog', array( $this, 'activate_new_site' ) );
		add_action( 'wp_enqueue_scripts', array( $this, 'enqueue_scripts' ) );

		add_action( 'widgets_init', create_function( '', 'register_widget("ChimpMate_WPMC_Assistant_Widget");' ) );

		add_action( 'wp_footer', array( $this, 'show_lightbox' ) );
		add_action( 'wp_head', array( $this, 'show_slider' ) );
		add_action( 'wp_footer', array( $this, 'show_topbar' ) );
		add_action( 'wp_footer', array( $this, 'show_flipbox' ) );
		add_filter('the_content', array( $this, 'addon_adder'));
		add_action( 'init', array($this,'register_shortcodes'));

		add_action( 'wp_footer', array( $this, 'social_connect' ) );
		add_action('wp_ajax_nopriv_chimpmate_get_opt_sup', array( $this, 'get_opt' ));

		add_action('wp_ajax_wpmchimpa_add_email_ajax', array( $this, 'subscribe' ));
		add_action('wp_ajax_nopriv_wpmchimpa_add_email_ajax', array( $this, 'subscribe' )); 

		add_action('wp_ajax_wpmchimpa_secret_ajax', array( $this, 'locked_secret' ));
		add_action('wp_ajax_nopriv_wpmchimpa_secret_ajax', array( $this, 'locked_secret' )); 

		add_filter( 'comment_form_field_comment', array($this,'commentfield' ));
		add_action( 'comment_post', array($this,'commentpost' ));

		add_action( 'register_form', array($this,'regfield' ));
		add_action( 'user_register',array($this,'regpost' ));

		add_action( 'wp_footer', array( $this, 'enqueue_font' ) );
	...
	public function integrateWithVC() {
		...
		if(isset($this->orig_set["addon_scode"])){
			vc_map( array(
				"name" => __("ChimpMate", 'vc_extend'),
				"description" => __("Subscription Box", 'vc_extend'),
				"base" => "chimpmate",
				"class" => "wpmcforce",
				"icon" => plugins_url('assets/icon/chimpmate.png', __FILE__),
	...
	public function load_plugin_textdomain() {
		...
		$this->settings = get_option('wpmchimpa_options');
	}
	public function loader(){
		...
		if(((isset($cmeta['Lightbox']['status'])&& $cmeta['Lightbox']['status'] == '1' ) || (!isset($cmeta['Lightbox']['status']) && isset($cmeta_cat['Lightbox']['status']) && $cmeta_cat['Lightbox']['status'] == '1') || ( !isset($cmeta_cat['Lightbox']['status']) && !isset($cmeta['Lightbox']['status']) && isset($this->settings["litebox"])&& $this->pagetype('l'))) && $this->user_status() && $this->referral() )
			$this->isload['l']=true;
		if(((isset($cmeta['Slider']['status'])&& $cmeta['Slider']['status'] == '1' ) || (!isset($cmeta['Slider']['status']) && isset($cmeta_cat['Slider']['status']) && $cmeta_cat['Slider']['status'] == '1') || ( !isset($cmeta_cat['Slider']['status']) && !isset($cmeta['Slider']['status']) && isset($this->settings["slider"])&& $this->pagetype('s'))) && $this->user_status() && $this->referral() )
			$this->isload['s']=true;
		if(((isset($cmeta['Addon']['status'])&& $cmeta['Addon']['status'] == '1') || (!isset($cmeta['Addon']['status']) && isset($cmeta_cat['Addon']['status']) && $cmeta_cat['Addon']['status'] == '1') || ( !isset($cmeta_cat['Addon']['status']) && !isset($cmeta['Addon']['status']) && isset($this->settings["addon"])&& $this->pagetype('a'))) && $this->user_status() && $this->referral() )
			$this->isload['a']=true;
		if(((isset($cmeta['Flipbox']['status'])&& $cmeta['Flipbox']['status'] == '1') || (!isset($cmeta['Flipbox']['status']) && isset($cmeta_cat['Flipbox']['status']) && $cmeta_cat['Flipbox']['status'] == '1') || ( !isset($cmeta_cat['Flipbox']['status']) && !isset($cmeta['Flipbox']['status']) && isset($this->settings["flipbox"])&& $this->pagetype('f'))) && $this->user_status() && $this->referral() )
			$this->isload['f']=true;
		if(((isset($cmeta['Topbar']['status'])&& $cmeta['Topbar']['status'] == '1') || (!isset($cmeta['Topbar']['status']) && isset($cmeta_cat['Topbar']['status']) && $cmeta_cat['Topbar']['status'] == '1') || ( !isset($cmeta_cat['Topbar']['status']) && !isset($cmeta['Topbar']['status']) && isset($this->settings["topbar"])&& $this->pagetype('t'))) && $this->user_status() && $this->referral() )
			$this->isload['t']=true;
		if((isset($cmeta['Widget']['status'])&& $cmeta['Widget']['status'] == '1')  || (!isset($cmeta['Widget']['status']) && isset($cmeta_cat['Widget']['status']) && $cmeta_cat['Widget']['status'] == '1') || ( !isset($cmeta_cat['Widget']['status']) && !isset($cmeta['Widget']['status']) && isset($this->settings["widget"])))
			$this->isload['w']=true;
		global $wpmc_font;
		$wpmc_font = array();
	}
	...
	public function enqueue_font(){
		global $wpmc_font;
		if(empty($wpmc_font))return false;
		$goo_fonts = array_diff(array_unique($wpmc_font), $this->webfont());
		if(!empty($goo_fonts)){
			$goo = implode('|', array_values($goo_fonts));
			wp_enqueue_style($this->plugin_slug . '-googleFonts', '//fonts.googleapis.com/css?family='.$goo, array(), self::VERSION);
		}
	}
	/**
	...
	public function getformbyid($id){
		if(!isset($this->settings['cforms']) || empty($this->settings['cforms']))return null;
		$this->curform = $id;
		if(isset($this->preform[$id]))return $this->preform[$id];
		$key = 0;
		foreach ($this->settings['cforms'] as $k => $v) {
			if($v['id'] == $id)
				$key = $k;
		}
		$this->preform[$id] = $this->settings['cforms'][$key];
		$this->preform[$id]['orig_field'] = $this->orig_set['cforms'][$key]['fields'];
		return $this->preform[$id];
	}
	/**
	 * Get Form by ID
	 * @since	1.0.0
	...
	public function getthemebyid($id){
		if(!isset($this->settings['themes']) || empty($this->settings['themes']))return null;
		foreach ($this->settings['themes'] as $k => $v) {
			if($v['id'] == $id)
				return $v;
		}
	...
	public function show_lightbox() {
		if(isset($this->isload['l'])){
			$this->loadscripts();
			include_once( 'includes/litebox_content.php' );
		}
	}
...
public function fieldset($f, $of = null){
	switch ($f['type']) {
		case 'email':
		...
		case 'dropdown':
			...
			if($f['cat'] == 'group'){
				...
				foreach ($f['opt']['choices'] as $k => $v) {
					$t.= '<option value="'.$v.'">'.$of['opt']['choices'][$k].'</option>';
				}
			}
...
public function stfield($fields,$set){
	...
	$ofield = $this->preform[$this->curform]['orig_field'];
	foreach ($fields as $k =>$f) {
		if(!isset($f['type']))continue;
		switch ($f['type']) {
			case 'hidden':
				echo (isset($f['value']) && $f['value'])? '<input type="hidden" name="group['.$f['value'].']" value="'.$f['value'].'">' : '';
				break;
			case 'email':
				if($set['type']==1)
	...
	public static function update_db_check(){
		...
		if(isset($wpmchimpa['slider_behave_movein'])){
			$dbc=true;
			$wpmchimpa['slider_transit'] = 2;
			unset($wpmchimpa['slider_behave_movein']);
		}
		if(!isset($wpmchimpa['slider_transit'])){
			$dbc=true;
			$wpmchimpa['slider_transit'] = 1;
		}
		if(isset($wpmchimpa['theme'])){
			$dbc=true;

			if(isset($wpmchimpa['api_key'])){
				$wpmchimpa['mailserv'] = array('key'=>$wpmchimpa['api_key']);
				unset($wpmchimpa['api_key']);
			}
			$cmt=array('lite','slider','widget','addon','topbar','flipbox');
			$sep=array(
				'close_col', 'bg_op', 'dislogo', 'head_col', 'hshad_col', 'disimg', 'img1', 'vid1', 'dissoc', 'exhead', 'exhopt', 'canvas_c', "bg_p", "trigger_i", "trigger_c", "trigger_bg", "trigger_top", "trigger_hider", "trigger_hc", 'bor_c', 'bor_w'
			);
			$font=array(
				'f'=>'f', 's'=>'fs', 'w'=>'fw', 'st'=>'fst'
			);
			$name=array(
				'l'=>'lightbox',
				's'=>'slider',
			...
			$tpl=array('Basic','Epsilon','Material','Onamy','Smash','Glow','Unidesign','Material Lite','Nova','Leo');
			$cmtc = array('Lightbox'=>'l', 'Slider'=>'s', 'Widget'=>'w', 'Addon'=>'a', 'Topbar'=>'a', 'Flipbox'=>'a');
			if(isset($wpmchimpa['cforms']) && !empty($wpmchimpa['cforms'])){
				$i=0;
				$nf=array();
				$ord=array();
				foreach ($wpmchimpa['cforms'] as $k => $v) {
				...
				$wpmchimpa['cformi'] = ''.$i;
				foreach ($cmt as $v) {
					$wpmchimpa[$v.'_form']=$ordf[$wpmchimpa[$v.'_form']];
				}
			}
			if(isset($wpmchimpa['theme']) && !empty($wpmchimpa['theme'])){
				$i=0;
				$nt=array();
				$ordt=array();
				foreach ($wpmchimpa['theme'] as $k => $v) {
					$i++;
					$t = str_split($k);
					if(!isset($ordt[$t[0]]))$ordt[$t[0]]=array();
					$ordt[$t[0]][$t[1]]=''.$i;
					$a=array();
					foreach ($v as $key => $value) {
						$j=0;
						foreach ($sep as $ka => $va) {
							if (strpos($value, $va) !== FALSE) {
								$j=1;
								break;
							}
						}
						...
						$nk = $key;
						foreach ($cmt as $va) {
							$nk = str_replace($va.'_', '', $nk);
						}
						foreach ($font as $kc => $va) {
							if(substr_compare($nk, $va, strlen($nk)-strlen($va), strlen($va)) === 0){
								$ka=explode('f', $nk);
								$kb=$ka[0].'f';
								if(!isset($a[$kb]))$a[$kb]=array();
								$a[$kb][$kc]=$value;
								$j=1;
								break;
							}
					...
					$b=array(
						"id"=>$i,
						"name"=>$tpl[intval($t[1])].'('.$name[$t[0]].')',
						"options"=>$a
					);
					array_push($nt, $b);
				}
				$wpmchimpa['themes']=$nt;
				$wpmchimpa['themehead']=''.$i;
				unset($wpmchimpa['theme']);
				if(isset($wpmchimpa['litebox_theme']))
					$wpmchimpa['lite_theme']=$ordt['l'][$wpmchimpa['litebox_theme']];
				if(isset($wpmchimpa['slider_theme']))
					$wpmchimpa['slider_theme']=$ordt['s'][$wpmchimpa['slider_theme']];
				if(isset($wpmchimpa['widget_theme']))
					$wpmchimpa['widget_theme']=$ordt['w'][$wpmchimpa['widget_theme']];
				if(isset($wpmchimpa['addon_theme']))
					$wpmchimpa['addon_theme']=$ordt['a'][$wpmchimpa['addon_theme']];
				if(isset($wpmchimpa['flipbox_theme']))
					$wpmchimpa['flipbox_theme']=$ordt['a'][$wpmchimpa['flipbox_theme']];
				if(isset($wpmchimpa['topbar_theme']))
					$wpmchimpa['topbar_theme']=$ordt['a'][$wpmchimpa['topbar_theme']];
			}

			$wpmchimpa['db_change']=1;

		}
		if($dbc)
			update_option('wpmchimpa_options',$wpmchimpa);
	}
	/**
...
public function social_connect(){
	...
	if(isset($this->settings['fb_api'])){
		?>
		<script>
		window.fbAsyncInit = function() {
			FB.init({
			appId	: '<?php echo $this->settings["fb_api"];?>',
		cookie : false, xfbml : true, version : 'v2.8'});};(function(d, s, id){ var js, fjs = d.getElementsByTagName(s)[0]; if (d.getElementById(id)) {return;} js = d.createElement(s); js.id = id; js.src = "//connect.facebook.net/en_US/sdk.js"; fjs.parentNode.insertBefore(js, fjs);}(document, 'script', 'facebook-jssdk'));
		</script>
		<?php
	}
	if(isset($this->settings['gp_api'])){
		?>
		<meta name="google-signin-clientid" content="<?php echo $this->settings["gp_api"];?>" />
	...
	if(isset($this->settings['ms_api'])){
		?>
		<script type="text/javascript" src="//js.live.net/v5.0/wl.js"></script>
		<script type="text/javascript">
		WL.init({
			client_id: '<?php echo $this->settings["ms_api"];?>',
...
function pagetype($e){
	switch ($e) {
		case 'l':
			if(isset($this->settings["lite_homepage"]) && is_front_page()) return true;
			else if(isset($this->settings["lite_blog"]) && !is_front_page() && is_home()) return true;
			else if(isset($this->settings["lite_page"]) && !is_front_page() && is_page()) return true;
			else if(isset($this->settings["lite_post"]) && is_single()) return true;
			else if(isset($this->settings["lite_category"]) && is_archive()) return true;
			else if(isset($this->settings["lite_search"]) && is_search()) return true;
			else if(isset($this->settings["lite_404error"]) && is_404()) return true;
			break;
		case 's':
			if(isset($this->settings["slider_homepage"]) && is_front_page()) return true;
			else if(isset($this->settings["slider_blog"]) && !is_front_page() && is_home()) return true;
			else if(isset($this->settings["slider_page"]) && !is_front_page() && is_page()) return true;
			else if(isset($this->settings["slider_post"]) && is_single()) return true;
			else if(isset($this->settings["slider_category"]) && is_archive()) return true;
			else if(isset($this->settings["slider_search"]) && is_search()) return true;
			else if(isset($this->settings["slider_404error"]) && is_404()) return true;
			break;	
		case 't':
			if(isset($this->settings["topbar_homepage"]) && is_front_page()) return true;
			else if(isset($this->settings["topbar_blog"]) && !is_front_page() && is_home()) return true;
			else if(isset($this->settings["topbar_page"]) && !is_front_page() && is_page()) return true;
			else if(isset($this->settings["topbar_post"]) && is_single()) return true;
			else if(isset($this->settings["topbar_category"]) && is_archive()) return true;
			else if(isset($this->settings["topbar_search"]) && is_search()) return true;
			else if(isset($this->settings["topbar_404error"]) && is_404()) return true;
			break;	
		case 'f':
			if(isset($this->settings["flipbox_homepage"]) && is_front_page()) return true;
			else if(isset($this->settings["flipbox_blog"]) && !is_front_page() && is_home()) return true;
			else if(isset($this->settings["flipbox_page"]) && !is_front_page() && is_page()) return true;
			else if(isset($this->settings["flipbox_post"]) && is_single()) return true;
			else if(isset($this->settings["flipbox_category"]) && is_archive()) return true;
			else if(isset($this->settings["flipbox_search"]) && is_search()) return true;
			else if(isset($this->settings["flipbox_404error"]) && is_404()) return true;
			break;	
		case 'a':
			if(isset($this->settings["addon_page"]) && is_page()) return true;
			else if(isset($this->settings["addon_post"]) && is_single()) return true;
	}
	return false;
}
...
function user_status() {
	...
	if(!isset($this->settings["loggedin"]) && !isset($this->settings["notloggedin"])) return false;
	if(!isset($this->settings["loggedin"]) && isset($this->settings["notloggedin"]) && is_user_logged_in()) return false;
	if(isset($this->settings["loggedin"]) && !isset($this->settings["notloggedin"]) && !is_user_logged_in()) return false;
	if(is_single() && is_user_logged_in()){
		global $wp_query;
		$comm = get_comments( array('post_id' => $wp_query->post->ID,'user_id' => get_current_user_id()));
		if(!isset($this->settings["commented"]) && !isset($this->settings["notcommented"])) return false;
		if(!isset($this->settings["commented"]) && isset($this->settings["notcommented"]) && !empty($comm)) return false;
		if(isset($this->settings["commented"]) && !isset($this->settings["notcommented"]) && empty($comm)) return false;
	}
	return true;
}
...
function referral() {
	...
	if(!isset($this->settings["searchengine"]))
		return true;
...
<?php
	 ...
	 * 
	 */
	public function subscribe($a,$l=null){
		$_POST = stripslashes_deep( $_POST );
		$settings=$this->settings;
		$form = $this->getformbyid($_POST['wpmcform']);
		$list = (($l == null) ? $form['list']['id'] : $l);
		if(!isset($settings['mailserv']['key']) || empty($list)){ if($a == 1)return;
					echo json_encode(array(
						'status' =>  0
					));
					die();}
		$MailChimp = new ChimpMate_WPMC_MailChimp($settings['mailserv']['key']);
		$email=$_POST['email'];
		...
		if($a == 1)return;
		if( $result['detail'] ){
			if(strpos($result['detail'],'already a list member') !== false){
				$gresult = $MailChimp->get('/lists/'.$list.'/members/'.md5($email));
				if($gresult['status'] == 'unsubscribed'){
					$result = $MailChimp->put('/lists/'.$list.'/members/'.md5($email),$set);
					echo json_encode(array(
						'status' =>  1
					));
				}
	...
	public function addon_adder($content){
		...
		if(isset($this->isload['a'])){
			...
			$msg = ob_get_clean();
			switch ($this->settings["addon_orient"]) {
...
function register_shortcodes(){
	if(isset($this->settings["addon_scode"]))
...
function addon_scode($atts, $content = null) {
	...
	private static function single_activate() {
		...
		if (function_exists('curl_init') && function_exists('curl_setopt')){
			...
			curl_setopt_array($curl, array(
				...
				CURLOPT_URL => 'http://voltroid.com/wpmchimpa/api.php',
	...
	public function iconlist(){
		return array(
			...
			'f01' => array(
				'path' => '<polygon points="83.155,234.108 102.504,306.111 130.978,306.111 166.979,194.202 137.708,194.202  116.353,267.542 97.629,194.202 69.048,194.202 49.742,267.542 28.775,194.202 0,194.202 35.419,306.111 64.238,306.111"/> <polygon points="265.472,194.202 236.912,194.202 217.584,267.542 196.531,194.202 167.756,194.202  203.24,306.111 232.08,306.111 251.019,234.108 270.411,306.111 298.842,306.111 334.843,194.202 305.636,194.202  284.173,267.542"/> <polygon points="473.392,194.202 451.973,267.542 433.292,194.202 404.668,194.202 385.34,267.542  364.395,194.202 335.577,194.202 371.082,306.111 399.879,306.111 418.818,234.108 438.146,306.111 466.662,306.111  502.664,194.202"/> <path d="M251.321,35.764c-86.564,0-161.026,51.166-195.28,124.765h390.582 C412.326,86.93,337.885,35.764,251.321,35.764z"/> <path d="M251.321,466.9c86.585,0,160.961-51.144,195.28-124.765H56.084 C90.338,415.734,164.758,466.9,251.321,466.9z"/>',
				'dim' => 502.664
			),
			'f01' => array(
				'path' => '<polygon points="83.155,234.108 102.504,306.111 130.978,306.111 166.979,194.202 137.708,194.202  116.353,267.542 97.629,194.202 69.048,194.202 49.742,267.542 28.775,194.202 0,194.202 35.419,306.111 64.238,306.111"/> <polygon points="265.472,194.202 236.912,194.202 217.584,267.542 196.531,194.202 167.756,194.202  203.24,306.111 232.08,306.111 251.019,234.108 270.411,306.111 298.842,306.111 334.843,194.202 305.636,194.202  284.173,267.542"/> <polygon points="473.392,194.202 451.973,267.542 433.292,194.202 404.668,194.202 385.34,267.542  364.395,194.202 335.577,194.202 371.082,306.111 399.879,306.111 418.818,234.108 438.146,306.111 466.662,306.111  502.664,194.202"/> <path d="M251.321,35.764c-86.564,0-161.026,51.166-195.28,124.765h390.582 C412.326,86.93,337.885,35.764,251.321,35.764z"/> <path d="M251.321,466.9c86.585,0,160.961-51.144,195.28-124.765H56.084 C90.338,415.734,164.758,466.9,251.321,466.9z"/>',
	...
	function myArrFilter($var){
		return (!empty($var));
	}
	/**
	 * Add field to comment box
	 * @since	1.0.0
	 * 
	 */
	public function commentfield( $comment_field ) {
		if(isset($this->settings['usyn_com']) && isset($this->settings['usyn_comp']) && $this->settings['usyn_comp']=='1')
			return $comment_field.'<label><input type="checkbox" name="wpmchimpa" value="1">'.(isset($this->settings['usyn_compt'])?$this->settings['usyn_compt']:__( 'Subscribe to Newsletters', 'chimpmate-wpmca' )).'</label>';
		return $comment_field;
	}
	/**
	 * Subscribe user from comment
	 * @since	1.0.0
	 * 
	 */
	public function commentpost($comment_ID){
		if(isset($this->settings['usyn_com']) && isset($this->settings['usyn_comp'])){
			if(($this->settings['usyn_comp']=='1' && isset ( $_POST['wpmchimpa'] )) || ($this->settings['usyn_comp']=='0')){
				if(is_user_logged_in()){
					global $current_user;
					get_currentuserinfo();
					$_POST['email']=$current_user->user_email;
					$_POST['merge_fields']['fname']=$current_user->user_firstname .' '. $current_user->user_lastname;
				}
				else
					$_POST['merge_fields']['fname']=$_POST['author'];
				$this->subscribe(1, $this->settings['usyn_coml']['id']);
			}
		}
	}
	/**
	 * Add field to Registration box
	 * @since	1.0.0
	 * 
	 */
	public function regfield(){
		if(isset($this->settings['usyn_reg']) && isset($this->settings['usyn_regp']) && $this->settings['usyn_regp']=='1')
			echo '<label><input type="checkbox" name="wpmchimpa" value="1">'.(isset($this->settings['usyn_regpt'])?$this->settings['usyn_regpt']:__( 'Subscribe to Newsletters', 'chimpmate-wpmca' )).'</label>';
	}
	/**
	 * Subscribe user from registration
	 * @since	1.0.0
	 * 
	 */
	public function regpost(){
		if(isset($this->settings['usyn_reg']) && isset($this->settings['usyn_regp'])){
			$current_user = wp_get_current_user();
			$roles = $current_user->roles;
			if(($this->settings['usyn_regp']=='1' && isset ( $_POST['wpmchimpa'] )) || ($this->settings['usyn_regp']=='0' && in_array($roles[0], $this->settings['usync_role']))){
				$_POST['wpmc']=array(
					'email' => $_POST['user_email']
				);
				$this->subscribe(1, $this->settings['usyn_regl']['id']);
			}
		}
	}
}
